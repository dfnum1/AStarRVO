...
namespace AStarPathfinding
{
    ...
    public class RVOAStarIntegrator
    {
        ...
        public RVOAStarIntegrator(AStar astar, UnitManager unitManager, int timeStep = 16)
        {
            m_astar = astar;
            m_unitManager = unitManager;
            m_cellSize = astar.Map.CellSize;
        ...
        public void DoStep()
        {
            ...
            foreach (var kvp in m_unitToAgentMap)
            {
                ...
                Vector3 velocity = new Vector3(
                    agent.Velocity.x / 100.0f,
                    0,
                    agent.Velocity.z / 100.0f
                );
                
                // 更新单位位置
                m_unitManager.UpdateUnitPosition(unitId, position);
                
                // 这里可以更新单位的速度，以便动画系统使用
